!! This file is the Grell-Freitas-Li deep convection scheme.

MODULE module_cu_gfl_deep
     !use machine , only : kind_phys
     !use physcons, only : qamin
     use module_cu_gfl_common, only : kind_phys
     real(kind=kind_phys), parameter:: qamin=1.e-32
...
!>\defgroup cu_gfl_deep_group Grell-Freitas-Li Deep Convection Module
!>\ingroup cu_gfl_group
!! This is Grell-Freitas-Li deep convection scheme module
!> @{
...
!! \section general_gf_deep Grell-Freitas-Li Deep Convection General Algorithm
   subroutine cu_gfl_deep_run(        &          
              ...
              ,us            &  ! u on mass points
              ,vs            &  ! v on mass points
              ,qc            &  ! qc on mass points
              ,qi            &  ! qi on mass points
              ,rho           &  ! density
              ...
              ,frh_out_far   &  ! fractional coverage - second row of grid points
...
!$acc declare create(xf_ens,pr_ens)
  ...
  ! pre    = output precip
     ...
     real(kind=kind_phys),    dimension (its:ite)                      &
        ,intent (inout  )                   ::                         &
        pre,xmb_out
!$acc declare copy(cnvwt,outu,outv,outt,outq,outqc,cupclw,frh_out,frh_out_far,pre,xmb_out)
     real(kind=kind_phys),    dimension (its:ite)                      &
        ,intent (in  )                   ::                            &
        mc_thresh,f_thresh,hfx,qfx,xmbm_in,xmbs_in
...
!
  ...
  ! hkb = moist static energy at originating level
     ...
     real(kind=kind_phys), dimension (kts:kte) :: trac,trcflx_in,sub_tend,trc,trco,dellac2
     real(kind=kind_phys), dimension (its:ite,kts:kte) :: pwdper
     integer :: nv
...
!$acc        u_cup,v_cup,uc,vc,ucd,vcd,dellu,dellv)
  ...
  ! edt     = epsilon
     ...
     real(kind=kind_phys),    dimension (its:ite) ::                     &
       ...
       cap_max_increment,closure_n,psum,psumh,sig,sigd,sig_init
     real(kind=kind_phys),    dimension (its:ite) ::                     &
...
!$acc declare copy(ierr) copyin(csum)
     ...
     real(kind=kind_phys)                 ::                             &
      ...
      dh,cap_maxs,trash,trash2,frh,frh_far,sig_thresh
...
!$acc declare create(zuh2,rntot,delqev,delq2,qevap,rn,qcond)
     ...
     real(kind=kind_phys) :: pgeoh,dts,fp,fpi,pmin,x_add,beta,beta_u,tot_clw
...
!---meltglac-------------------------------------------------
     ...
     real(kind=kind_phys),    dimension (kts:kte) :: c_local,p_local,zd_local,massflx,h_local
...
!$acc parallel loop private(beta,itemp,dzo,h_entr)
      do i=its,itf
        ...
        zdo(i,kts:jmin(i))=zd_local(kts:jmin(i))
...
123     format(a7,1x,3i3,2e12.4,1(1x,f5.2),3e12.4)
      ...
      enddo
! END MASS check
      if (sub3d == 0 ) then ! no subsidence split
        do i=its,itf
          if(ierr(i)/=0)cycle
           dp=100.*(po_cup(i,1)-po_cup(i,2))
      ...
      do i=its,itf
        ...
        if(use_fct == 0 ) then
          ...
          enddo
        else   ! use_fct=1 is preferred choice !
          ...
          h_local(kts:kte)=heo_cup(i,kts:kte)
          do k=kts,ktop(i)
            dp=100.*(po_cup(i,k)-po_cup(i,k+1))
            massflx   (k) =-(zuo(i,k)  -edto(i)*zdo(i,k))
            trcflx_in (k) =massflx(k)*heo_cup(i,k)
            dtime_max=min(dtime_max,.5*dp)
          enddo
          call fct1d3 (ktop(i),kte,dtime_max,p_local,h_local,massflx,trcflx_in(:),sub_tend(:),g)
        ...
        if(use_fct == 0 ) then
          ...
          trcflx_in(:) = 0. ! dummy array
          p_local(kts:kte)=po_cup(i,kts:kte)
          h_local(kts:kte)=qo_cup(i,kts:kte)
          massflx  (:) = 0.
          dtime_max      = dtime
          ...
          do k=kts,ktop(i)
            dp=100.*(po_cup(i,k)-po_cup(i,k+1))
            massflx   (k) =-(zuo(i,k)  -edto(i)*zdo(i,k))
            trcflx_in (k) =massflx(k)*qo_cup(i,k)
            dtime_max=min(dtime_max,.5*dp)
          enddo
          call fct1d3 (ktop(i),kte,dtime_max,p_local,h_local,massflx,trcflx_in,sub_tend(:),g)
        endif
...
! initialize fct routine
         do k=kts,ktop(i)
            ...
            massflx(k)=-xmb(i)*(zuo(i,k)-edto(i)*zdo(i,k))
            trcflx_in(k)=massflx(k)*chem_cup(i,k,nv)
            c_local(k)=chem(i,k,nv)
         enddo
         trcflx_in(1)=0.
         massflx(1)=0.
         p_local(kts:kte)=po_cup(i,kts:kte)
         call fct1d3(ktop(i),kte,dtime_max,p_local,c_local,massflx,   &
                     trcflx_in,dellac2,g)
         do k=kts,ktop(i)
             trash=chem (i,k,nv)
             chem (i,k,nv)=chem (i,k,nv) + (dellac(i,k,nv)+dellac2(k))*dtime
...
!$acc atomic
      ...
      do i = its,itf
         ...
         flg(i) = .true.
         if(ierr(i).eq.0)then
         evef = edt(i) * evfact * sig_init(i)**2
         if(xland(i).gt.0.5 .and. xland(i).lt.1.5) evef = edt(i) * evfactl * sig_init(i)**2
!$acc loop seq
         do k = ktop(i), 1, -1
              rain =  pwo(i,k) + edto(i) * pwdo(i,k)
...
!
   ...
   end subroutine cu_gfl_deep_run
...
!             if(forcing(i,1).lt.0.4)ierr(i)=2242
!             if(forcing(i,1).lt.0.4)cycle
!   
...
!$acc declare create(hcot)
     ...
     real(kind=kind_phys) :: dby(kts:kte),dbm(kts:kte),zux(kts:kte),p_local(kts:kte),zu_local(kts:kte)
...
!        ktop(i)=kfinalzu
        if(kfinalzu.le.kbcon(i)+2)then
           ...
           call get_zu_zd_pdf_fim(kklev,p_local,rand_vmas(i),zubeg,ipr,xland(i),zuh2,1,ierr(i),k22(i), &
            ...
            zuo(i,kts:ktf)=zu_local(kts:ktf)
        endif
      endif ! end deep
      if ( is_mid ) then
